...
def tabledebord(request):
    ...
    total_reclamations_traitees = Reclamation.objects.filter(status='Traitee').count()
    total_reclamations_en_attente = Reclamation.objects.filter(status='En attente').count()
    total_matiere = Matiere.objects.all().count()

    return render(request, 'agent_scolarite/TableDeBord.html',locals())
